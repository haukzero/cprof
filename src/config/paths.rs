...
use crate::error::{AppError, IoContext, PathError, Result, TargetError};

/// Validate a value that is used as one directory component below cprof's
/// profile root.
pub(crate) fn validate_target_id(value: &str) -> Result<()> {
    validate_safe_component(value).map_err(|()| TargetError::InvalidId(value.to_string()).into())
}
...
pub(crate) fn validate_filename(value: &str) -> Result<()> {
    if is_windows_reserved_component(value) {
        return Err(PathError::InvalidFilename(value.to_string()).into());
    }
    validate_safe_component(value)
        .map_err(|()| PathError::InvalidFilename(value.to_string()).into())
}
...
pub(crate) fn relative_active_path(value: &str) -> Result<PathBuf> {
    if value.is_empty()
        ...
        || has_windows_prefix(value)
    {
        return Err(PathError::InvalidActivePath(value.to_string()).into());
    }
...
fn join_relative(root: &Path, relative: &Path) -> Result<PathBuf> {
    if !root.is_absolute() {
        return Err(PathError::Unsafe(root.display().to_string()).into());
    }
    ...
    {
        return Err(PathError::Unsafe(relative.display().to_string()).into());
    }

    let root = lexical_normalize(root)?;
    let candidate = lexical_normalize(&root.join(relative))?;
    if !candidate.starts_with(&root) || candidate == root {
        return Err(PathError::Unsafe(candidate.display().to_string()).into());
    }
    Ok(candidate)
}
...
pub(crate) fn checked_absolute_active(value: &str) -> Result<PathBuf> {
    ...
    let parent = path
        .parent()
        .ok_or_else(|| PathError::InvalidActivePath(value.to_string()))?;
    ...
    match (relative, absolute) {
        ...
        (Some(relative), Some(absolute)) => {
            if path_identity(&relative)? != path_identity(&absolute)? {
                return Err(PathError::InvalidActivePath(
        ...
        (None, None) => Err(PathError::InvalidActivePath(
...
pub(crate) fn path_identity(path: &Path) -> Result<PathBuf> {
    let parent = path
        .parent()
        .ok_or_else(|| PathError::Unsafe(path.display().to_string()))?;
    let filename = path
        .file_name()
        .ok_or_else(|| PathError::Unsafe(path.display().to_string()))?;
